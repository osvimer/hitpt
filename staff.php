...
//--------------------- film critics section ---------------------------//
//2012-5-29新添加用户标签：保种组、外联组、美工组
//--------------------- 保种组 ---------------------------//
unset($ppl);
$res = sql_query("SELECT * FROM users WHERE users.seedkeeper='yes' AND status='confirmed' ORDER BY username") or sqlerr();
while ($arr = mysql_fetch_assoc($res))
{
	$countryrow = get_country_row($arr['country']);
	$ppl .= "<tr><td class=embedded>". get_username($arr['id']) ."</td><td class=embedded><img width=24 height=15 src=\"pic/flag/".$countryrow[flagpic]."\" title=\"".$countryrow['name']."\" style=\"padding-bottom:1px;\"></td>
 <td class=embedded> ".(strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg)."</td>".
 "<td class=embedded><a href=sendmessage.php?receiver=".$arr['id']." title=\"".$lang_staff['title_send_pm']."\">".$sendpmimg."</a></td>".
 "<td class=embedded>".$arr['seedfor']."</td></tr>\n";
}

begin_frame($lang_staff['text_seedkeeper']."<font class=small> - [<a class=altlink href=contactstaff.php><b>".$lang_staff['text_apply_for_it']."</b></a>]</font>");
?>
<?php echo $lang_staff['text_seedkeeper_note'] ?>
<br /><br />
<table width=100% cellspacing=0 align=center>
	<tr>
		<td class=embedded><b><?php echo $lang_staff['text_username'] ?></b></td>
		...
		<td class=embedded align=center><b><?php echo $lang_staff['text_online_or_offline'] ?></b></td>
		<td class=embedded align=center><b><?php echo $lang_staff['text_contact'] ?></b></td>
		<td class=embedded><b><?php echo $lang_staff['text_seedfor'] ?></b></td>
	</tr>
	<tr>
		<td class=embedded colspan=6>
			<hr color="#4040c0">
...
//--------------------- 外联组 ---------------------------//
unset($ppl);
$res = sql_query("SELECT * FROM users WHERE users.ambassador='yes' AND status='confirmed' ORDER BY username") or sqlerr();
while ($arr = mysql_fetch_assoc($res))
{
	$countryrow = get_country_row($arr['country']);
	$ppl .= "<tr><td class=embedded>". get_username($arr['id']) ."</td><td class=embedded><img width=24 height=15 src=\"pic/flag/".$countryrow[flagpic]."\" title=\"".$countryrow['name']."\" style=\"padding-bottom:1px;\"></td>
 <td class=embedded> ".(strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg)."</td>".
 "<td class=embedded><a href=sendmessage.php?receiver=".$arr['id']." title=\"".$lang_staff['title_send_pm']."\">".$sendpmimg."</a></td>".
 "<td class=embedded>".$arr['ambassadorfor']."</td></tr>\n";
}

begin_frame($lang_staff['text_ambassador']."<font class=small> - [<a class=altlink href=contactstaff.php><b>".$lang_staff['text_apply_for_it']."</b></a>]</font>");
?>
<?php echo $lang_staff['text_ambassador_note'] ?>
<br /><br />
<table width=100% cellspacing=0 align=center>
	<tr>
		<td class=embedded><b><?php echo $lang_staff['text_username'] ?></b></td>
		...
		<td class=embedded align=center><b><?php echo $lang_staff['text_online_or_offline'] ?></b></td>
		<td class=embedded align=center><b><?php echo $lang_staff['text_contact'] ?></b></td>
		<td class=embedded><b><?php echo $lang_staff['text_ambassadorfor'] ?></b></td>
	</tr>
	<tr>
		<td class=embedded colspan=6>
			<hr color="#4040c0">
...
//--------------------- 美工组 ---------------------------//
unset($ppl);
$res = sql_query("SELECT * FROM users WHERE users.artmember='yes' AND status='confirmed' ORDER BY username") or sqlerr();
while ($arr = mysql_fetch_assoc($res))
{
	$countryrow = get_country_row($arr['country']);
	$ppl .= "<tr><td class=embedded>". get_username($arr['id']) ."</td><td class=embedded><img width=24 height=15 src=\"pic/flag/".$countryrow[flagpic]."\" title=\"".$countryrow['name']."\" style=\"padding-bottom:1px;\"></td>
 <td class=embedded> ".(strtotime($arr['last_access']) > $dt ? $onlineimg : $offlineimg)."</td>".
 "<td class=embedded><a href=sendmessage.php?receiver=".$arr['id']." title=\"".$lang_staff['title_send_pm']."\">".$sendpmimg."</a></td>".
 "<td class=embedded>".$arr['artmemberfor']."</td></tr>\n";
}

begin_frame($lang_staff['text_artmember']."<font class=small> - [<a class=altlink href=contactstaff.php><b>".$lang_staff['text_apply_for_it']."</b></a>]</font>");
?>
<?php echo $lang_staff['text_artmember_note'] ?>
<br /><br />
<table width=100% cellspacing=0 align=center>
	<tr>
		<td class=embedded><b><?php echo $lang_staff['text_username'] ?></b></td>
		...
		<td class=embedded align=center><b><?php echo $lang_staff['text_online_or_offline'] ?></b></td>
		<td class=embedded align=center><b><?php echo $lang_staff['text_contact'] ?></b></td>
		<td class=embedded><b><?php echo $lang_staff['text_artmemberfor'] ?></b></td>
	</tr>
	<tr>
		<td class=embedded colspan=6>
			<hr color="#4040c0">
...
unset($ppl);
$res = sql_query("SELECT * FROM users WHERE class=".UC_VIP." AND status='confirmed' AND (stafffor is not null AND stafffor<>'') ORDER BY username") or sqlerr();
while ($arr = mysql_fetch_assoc($res))
{
	$countryrow = get_country_row($arr['country']);
	$ppl .= "<tr><td class=embedded>". get_username($arr['id']) ."</td><td class=embedded><img width=24 height=15 src=\"pic/flag/".$countryrow['flagpic']."\" title=\"".$countryrow['name']."\" style=\"padding-bottom:1px;\"></td>
