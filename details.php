...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class && !$owned)
	...
	if (!isset($_GET["cmtpage"])) {
		...
		print("<table width=\"1100\" cellspacing=\"0\" cellpadding=\"5\">\n");
		...
		if ($CURUSER["downloadpos"] != "no")
		{
			...
			print($lang_details['row_mouseclick']."<a class=\"index\" href=\"utorrent3.0.rar\">"."μTorrent"."</a>".$lang_details['row_openfile']);
			print("</td></tr>");
		}
		else
	...
	$douban_id = parse_douban_id($row["dburl"]);
		if ($douban_id)
		{
			$where_area = " dburl = " . sqlesc((int)$douban_id) ." AND torrents.id != ".sqlesc($id);
...
echo "</script>";
		...
		tr($lang_details['row_thanks_by'],"<span>".$lang_details['text_thanks_note1'].$saythanks_bonus.$lang_details['text_thanks_note2'].$receivethanks_bonus.$lang_details['text_thanks_note3']."</span><span id=\"thanksadded\" style=\"display: none;\"><input class=\"btn\" type=\"button\" value=\"".$lang_details['text_thanks_added']."\" disabled=\"disabled\" /></span><span id=\"curuser\" style=\"display: none;\">".get_username($CURUSER['id'])." </span><span id=\"thanksbutton\">".$thanksbutton."</span>&nbsp;&nbsp;<span id=\"nothanks\">".$nothanks."</span><span id=\"addcuruser\"></span>".$thanksby.($thanks_all < $thanksCount ? $lang_details['text_and_more'].$thanksCount.$lang_details['text_users_in_total'] : ""),1);
