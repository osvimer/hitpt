...
if ($_GET["add_offer"]){
	...
	"<tr><td class=rowhead align=right><b>".$lang_offers['row_title']."<font color=red>*</font></b></td><td class=rowfollow align=left><input type=text name=name style=\"width: 650px;\" /><br /><font class=\"medium\">".$lang_offers['text_title_note']."</font></td></tr>".
	"<tr><td class=rowhead align=right><b>".$lang_offers['row_small_description']."<font color=red>*</font></b></td><td class=rowfollow align=left><input type=text name=small_descr style=\"width: 650px;\" /><br /><font class=\"medium\">".$lang_offers['text_small_description_note']."</font></td></tr>".
...
if ($_GET["new_offer"]){
	...
	$small_descr = trim($_POST["small_descr"]);
	if ($small_descr == "")
		bark($lang_offers['std_must_enter_small_description']);
	...
	$res = sql_query("SELECT name FROM offers WHERE name =".sqlesc($name)) or sqlerr(__FILE__,__LINE__);
	...
	if (!$arr['name']){
		...
		$ret = sql_query("INSERT INTO offers (userid, name, small_descr, descr, category, added) VALUES (" .
		implode(",", array_map("sqlesc", array($CURUSER["id"], $name, $small_descr, $descr, 0 + $_POST["type"]))) .
		", '" . date("Y-m-d H:i:s") . "')");
		if (!$ret) {
			if (mysql_errno() == 1062)
			bark("!!!");
...
if ($_GET["off_details"]){
	...
	$s = $num["name"];
	$small_description = $num["small_descr"];
	...
	tr($lang_offers['row_small_description'], htmlspecialchars($small_description), 1);
...
if ($_GET["edit_offer"]) {
	...
	$small_description = htmlspecialchars(trim($small_description));
	...
	tr($lang_offers['row_title']."<font color=\"red\">*</font>", "<input type=\"text\" style=\"width: 650px\" name=\"name\" value=\"".$title."\" /><br /><font class=\"medium\">".$lang_offers['text_title_note']."</font>", 1);
	tr($lang_offers['row_small_description']."<font color=\"red\">*</font>", "<input type=\"text\" style=\"width: 650px\" name=\"small_descr\" value=\"".$small_description."\" /><br /><font class=\"medium\">".$lang_offers['text_small_description_note']."</font>", 1);
...
if ($_GET["take_off_edit"]){
	...
	$name = sqlesc($name);
	$small_descr = sqlesc($small_descr);
	$descr = sqlesc($descr);
	$cat = sqlesc($cat);

	sql_query("UPDATE offers SET category=$cat, name=$name, small_descr=$small_descr, descr=$descr where id=".sqlesc($id));
...
print("<div align=\"center\"><form method=\"get\" action=\"torrents.php\">".$lang_offers['text_search_offers']."&nbsp;&nbsp;<input type=\"text\" id=\"specialboxg\" name=\"search\" />&nbsp;&nbsp;");
$cats = genrelist($browsecatmode);
$catdropdown = "";
foreach ($cats as $cat) {
