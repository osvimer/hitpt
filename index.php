...
?>
<script type="text/javascript">
	jAlert('<font color=red>连续登录<?=$salarynum?>天奖励，恭喜你获取了<?=$addbonus?>点魔力值，继续保持哦</font>', '每日登录奖励');
</script>
<?
}else if($Days > 0){
...
?>
<script type="text/javascript">
	jAlert('<font color=red>每日登录奖励，恭喜你获取了<?=$addbonus?>点魔力值，连续多天登录会有更多奖励哦</font>', '每日登录奖励');
</script>
<?
		}
	}
...
if(!ipv6ip(findip()))
  ...
  print("<div align=center>".$lang_index['text_is_ipv6'].findip().$lang_index['text_visit_here']."</div>");
print("<div align=center>".$lang_index['text_download_ut']."</div>");
//IPV6引入---end---
...
{
	$Cache->add_whole_row();
	print("<table width=\"100%\"><tr><td class=\"text\"><div id=\"index_text\">\n");
	$Cache->end_whole_row();
	$news_flag = 0;
			...
			if (!$Cache->get_page())
			{
				...
				if (mysql_num_rows($res) > 0)
				{
					...
					while($array = mysql_fetch_array($res))
					{
						...
						if ($douban_id = parse_douban_id($array["dburl"]))
						{
...
<?php
	...
	print("<div style=\"width: 88%;\" id=\"slider-stage2\">");
...
//check whether current user has voted
	...
	print("<iframe src=\"fun.php?action=view\" width='1100' height='300' frameborder='0' name='funbox' marginwidth='0' marginheight='0'></iframe><br /><br />\n");
...
?>
<h2><?php echo $lang_index['text_shoutbox'] ?><a name="shoutbox1" id="shoutbox1"> </a>- <font class="small"><?php echo $lang_index['text_auto_refresh_after']?></font><font class='striking' id="countdown"></font><font class="small"><?php echo $lang_index['text_seconds']?></font></h2>
<?php
	print("<table width=\"100%\"><tr><td class=\"text\">\n");
	print("<iframe src='shoutbox.php?type=shoutbox' width='1100' height='360' frameborder='0' name='sbox' marginwidth='0' marginheight='0'></iframe><br /><br />\n");
	print("<form action='shoutbox.php' method='get' target='sbox' name='shbox'>\n");
	print("<label for='shbox_text'>".$lang_index['text_message']."</label><input type='text' name='shbox_text' id='shbox_text' size='100' style='width: 900px; border: 1px solid gray;' />  <input type='submit' id='hbsubmit' class='btn' name='shout' value=\"".$lang_index['sumbit_shout']."\" />");
...
<!-- 
<div align="center"><a href="http://pt.hit.edu.cn" title="<?php echo PROJECTNAME?>" target="_blank"><img src="pic/nexus.png" alt="<?php echo PROJECTNAME?>" /></a></div>
-->
<div align="center"><a href="http://pt.hit.edu.cn" title="<?php echo PROJECTNAME?>" target="_blank"></a></div>
</td></tr></table>
<?php
