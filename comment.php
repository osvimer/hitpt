...
if ($action == "add")
{
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST")
	{
		...
		if($type == "torrent"){
			...
			if($at_text)
			{
			$at_title = "<a href=details.php?id=$parent_id>".$arr['name']."</a>";
			$at_text = $CURUSER["username"].$lang_comment['text_at_you'].":  ".htmlspecialchars($at_text);
			at_user_message($at_text, $at_title, $type);
			}
		}
		elseif($type == "offer"){
			...
			if($at_text)
			{
			$at_title = "<a href=offers.php?id=$parent_id&off_details=1>".$arr['name']."</a>";
			$at_text = $CURUSER["username"].$lang_comment['text_at_you'].":  ".htmlspecialchars($at_text);
			at_user_message($at_text, $at_title, $type);
			}
		}
		elseif($type == "request")
		{
			...
			if($at_text)
			{
			$at_title = "<a href=viewrequests.php?id=$parent_id&req_details=1>".$arr['name']."</a>";
			$at_text = $CURUSER["username"].$lang_comment['text_at_you'].":  ".htmlspecialchars($at_text);
			at_user_message($at_text, $at_title, $type);
			}
		}
