...
$subject_text = specialchars_to_text($subject);
...
// Display message
$subject_text = specialchars_to_text($subject);
stdhead("PM ($subject_text)");
$subject_normal = specialchars_to_normal($subject);
?>
<h1><?php echo $subject_normal; ?></h1>
<?php
...
function specialchars_to_normal($value)
{
	$pattern = '/&lt;(a\shref=.*\d+)&gt;([\s\S]*)&lt;\/a&gt;/';
	$normal_val = preg_replace($pattern, "<"."$1".">"."$2"."</a>", $value);
	return $normal_val;
}

function specialchars_to_text($value)
{
	$pattern = '/&lt;a\shref=.*\d+&gt;([\s\S]*)&lt;\/a&gt;/';
	$text_val = preg_replace($pattern, "$1", $value);
	return $text_val;
}
