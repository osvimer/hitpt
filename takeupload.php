...
$largepro_flag = 0; //Oven: if promoted because of large torrent.
if ($largesize_torrent && $totallen > ($largesize_torrent * 1073741824)) //Large Torrent Promotion
{
	...
	$largepro_flag = 1; //Oven: have been promoted because of large torrent.
}
...
if($largepro_flag == 0)
{
	switch($catid)
	{
		case 401: //movie
		{
		    if($moviepro_torrent > 0)
			$sp_state = 0 + $moviepro_torrent;
		    break;
		}
		case 402: //tvshow
		{
		    if($tvshowpro_torrent > 0)
			$sp_state = 0 + $tvshowpro_torrent;
		    break;
		}
		case 405: //cartoon
		{
		    if($cartoonpro_torrent > 0)
			$sp_state = 0 + $cartoonpro_torrent;
		    break;
		}
		case 404: //documentary
		{
		    if($documentarypro_torrent > 0)
			$sp_state = 0 + $documentarypro_torrent;
		    break;
		}
		case 403: //variety
		{
		    if($varietypro_torrent > 0)
			$sp_state = 0 + $varietypro_torrent;
		    break;
		}
		case 407: //sports
		{
		    if($sportspro_torrent > 0)
			$sp_state = 0 + $sportspro_torrent;
		    break;
		}
		case 406: //music
		{
		    if($musicpro_torrent > 0)
			$sp_state = 0 + $musicpro_torrent;
		    break;
		}
		case 413: //software
		{
		    if($softwarepro_torrent > 0)
			$sp_state = 0 + $softwarepro_torrent;
		    break;
		}
		case 410: //game
		{
		    if($gamepro_torrent > 0)
			$sp_state = 0 + $gamepro_torrent;
		    break;
		}
		case 411: //study
		{
		    if($studypro_torrent > 0)
			$sp_state = 0 + $studypro_torrent;
		    break;
		}
		case 409: //others
		{
		    if($otherspro_torrent > 0)
			$sp_state = 0 + $otherspro_torrent;
		    break;
		}
		default: //non-exist category
		{
			break;
		}
	}
...
write_log("种子 $id ($torrent) 被上传,操作者 $anon");
