...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if (!$torrentid || !in_array($type,array('firsttime', 'client', 'ratio', 'bonus')))
		die("error");
...
else
{
	...
	switch ($type)
	{
		...
		case 'bonus':
		{
			$title = $lang_downloadnotice['text_low_bonus_notice'];
			$leechwarnuntiltime = strtotime($CURUSER['leechwarnuntil']);
			if (TIMENOW < $leechwarnuntiltime){
				$kicktimeout = gettime($CURUSER['leechwarnuntil'], false, false, true);
			}
			$note = $lang_downloadnotice['text_low_bonus_note_one'];
			$noticenexttime = $lang_downloadnotice['text_bonus_always_show'];
			$showrationotice = false;
			$showclientnotice = false;
