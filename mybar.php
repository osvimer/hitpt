...
if (!$_GET['noname'])
{
	...
	//Oven: fix a bug. Use imagestring cannot output Chinese text.
	//imagestring($my_img, $namesize, $namex, $namey, $username, $name_colour);
	$font = 'fonts/simhei.ttf';
	$pic = imagecreate(140, 20); 
	$grey = imagecolorallocate($pic, 130, 130, 130);
	$tran = imagecolortransparent($pic, $grey);
	$name_colour = imagecolorallocate($pic, $namered, $namegreen, $nameblue);
	imagettftext($pic, $namesize, 0, $namex, $namey, $name_colour, $font, $username);
