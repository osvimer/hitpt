...
function get_pick_str($pick_id)
{
    $pickstr = "";
    switch($pick_id)
    {
	case 1: $pickstr = "'hot'"; break;
	case 2: $pickstr = "'classic'"; break;
	case 3: $pickstr = "'recommended'"; break;
	default:$pickstr = NULL; break;
    }
    return $pickstr;
}
...
if($_GET["picktype"]) $picktype_get = 0 + $_GET["picktype"];  // 1 for hot, 2 for classic, 3 for recommend
...
if (!$all)
{
	if (!$_GET && $CURUSER['notifs'])
	{
		...
		foreach ($picktypes as $picktype)
		{
		    $all &= $picktype;
    			$pick_str = get_pick_str($picktype);
			if($pick_str) {
			$wherepickina[] = $pick_str;
			$addparam .= "picktype$picktype=1&";
			}
		}
		if ($showprocessing)
	...
	elseif ($picktype_get)
	{
    		int_check($picktype_get,true,true,true);
    		$pick_str = get_pick_str($picktype_get);
		if($pick_str) {
			$wherepickina[] = $pick_str;
			$addparam .= "picktype=$picktype_get&";
		}
	}
	...
	else	//select and go
	{
		...
		foreach ($picktypes as $picktype)
		{
		    $all &= $_GET["picktype$picktype"];
		    if ($_GET["picktype$picktype"])
		    {
    			$pick_str = get_pick_str($picktype);
			if($pick_str) {
			$wherepickina[] = $pick_str;
...
if (count($wherepickina) > 1)
$wherepickin = implode(",",$wherepickina);
elseif (count($wherepickina) == 1)
$wherea[] = "picktype = ".$wherepickina[0];
...
if ($wherepickin)
$where .= ($where ? " AND " : "") . "picktype IN(" . $wherepickin . ")";
...
print("<table width=\"1200\" class=\"main\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\">");
					...
					<?php
						function printcat($name, $listarray, $cbname, $wherelistina, $btname, $showimg = false)
						{
							...
							foreach($listarray as $list){
								...
								/*下面两行是用于显示种子类型的代码，区别在于第一行只有类型图片不带复选框，而第二行带复选框*/
						...
						/*下面这个printcatone函数用于显示不同类型的子类型，如“电影-1080P”*/
...
<?php
	print("<br />");
	print("<tr><td class=\"embedded\" colspan=\"".$catsperrow."\" align=\"left\" style=\"margin-bottom: 10px;\"><b>".$lang_torrents['text_picktype']."</b></td></tr>");
	print("<tr>");
	print("<td class=\"bottom\" style=\"margin-top:10px; padding-bottom: 4px; width: 80px; padding-left: ".$catpadding."px;\"><input type=\"checkbox\" id=\"picktype1\" name=\"picktype1\"" . (in_array(get_pick_str(1), $wherepickina) ? " checked=\"checked\"" : "") . " value=\"1\" /><b>[<a title=\"".$lang_torrents['text_hot']."\" href=\"?picktype=1\"><font class='hot'>".$lang_torrents['text_hot']."</font></a>]</b></td>");
	print("<td class=\"bottom\" style=\"margin-top:10px; padding-bottom: 4px; width: 80px; padding-left: ".$catpadding."px;\"><input type=\"checkbox\" id=\"picktype2\" name=\"picktype2\"" . (in_array(get_pick_str(2), $wherepickina) ? " checked=\"checked\"" : "") . " value=\"1\" /><b>[<a title=\"".$lang_torrents['text_classic']."\" href=\"?picktype=2\"><font class='classic'>".$lang_torrents['text_classic']."</font></a>]</b></td>");
	print("<td class=\"bottom\" style=\"margin-top:10px; padding-bottom: 4px; width: 80px; padding-left: ".$catpadding."px;\"><input type=\"checkbox\" id=\"picktype3\" name=\"picktype3\"" . (in_array(get_pick_str(3), $wherepickina) ? " checked=\"checked\"" : "") . " value=\"1\" /><b>[<a title=\"".$lang_torrents['text_recommended']."\" href=\"?picktype=3\"><font class='recommended'>".$lang_torrents['text_recommended']."</font></a>]</b></td>");
	print("</tr>");
?>
				</table>
			</td>
