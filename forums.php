...
if ($action == "post")
{
	...
	else
	{
		...
		if(trim($subject) == "")
		    $subject_name = get_single_value("topics","subject","WHERE id=".sqlesc($topicid));
		else
		    $subject_name = $subject;
		$at_text = is_include_at($body);
		if($at_text)
		{
		$at_title = "<a href=forums.php?action=viewtopic&forumid=$forumid&topicid=$topicid>".htmlspecialchars($subject_name)."</a>";
		$at_text = $CURUSER["username"].$lang_forums['text_at_you'].":  ".htmlspecialchars($at_text);
		at_user_message($at_text, $at_title, "topic");
		}
...
if ($action == "viewtopic")
{
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		$tmp_username = get_username($posterid,false,false,false,false,false,false,"",false,true);
		if ($maypost)
		{
		print("<a onclick=\"javascript:quick_reply_to('".$tmp_username."','".$floor_num."');\" href=\"#quickreply\"><img class=\"f_reply\" src=\"pic/trans.gif\" alt=\"Reply\" title=\"回复该用户\" /></a>");
...
<option style='background-color: white' value=\"40\">White</option>");
	...
	print($pagerbottom);
	if ($maypost){
	print("<br /><table style='border:1px solid #000000;'><a name=\"quickreply\" id=\"quickreply\"> </a><tr>".
