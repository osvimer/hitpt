...
$seedkeeper = $user["seedkeeper"] == 'yes';
$ambassador = $user["ambassador"] == 'yes';
$artmember = $user["artmember"] == 'yes';
...
$rs_true_trans = sql_query("SELECT SUM(uploaded), SUM(downloaded) FROM snatched WHERE userid = $user[id]") or sqlerr();
$true_download = 0;
$true_upload = 0;
...
$true_xfer = "<tr><td class=\"embedded\"><strong>" . $lang_userdetails['row_real_uploaded'] . "</strong>:  ". mksize($true_upload) . "</td><td class=\"embedded\">&nbsp;&nbsp;<strong>" . $lang_userdetails['row_real_downloaded'] . "</strong>:  " . mksize($true_download) . "</td><td class=\"embedded\">&nbsp;&nbsp;" . $lang_userdetails['row_real_ps'] . "</td></tr>";
...
print( ("'".$user['last_access']."'" > $dt ? "<img class=\"f_online\" src=\"pic/trans.gif\" alt=\"Online\" title=\"".$lang_userdetails['title_online']."\" />  " : "<img class=\"f_offline\" src=\"pic/trans.gif\" alt=\"Offline\" title=\"".$lang_userdetails['title_offline']."\" />  ") );
...
if (get_user_class() >= $prfmanage_class && $user["class"] < get_user_class())
{
	...
	$seedfor = htmlspecialchars($user["seedfor"]);
	$ambassadorfor = htmlspecialchars($user["ambassadorfor"]);
	$artmemberfor = htmlspecialchars($user["artmemberfor"]);
...
//服务组标签 start
tr($lang_userdetails['row_seedkeeper'], "<input type=\"radio\" name=\"seedkeeper\" value=\"yes\"" .($user["seedkeeper"] == "yes" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_yes']." <input type=\"radio\" name=\"seedkeeper\" value=\"no\"" .($user["seedkeeper"] == "no" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_no'], 1);
	tr($lang_userdetails['row_seedfor'], "<textarea cols=\"60\" rows=\"6\" name=\"seedfor\">".$seedfor."</textarea>", 1);

tr($lang_userdetails['row_ambassador'], "<input type=\"radio\" name=\"ambassador\" value=\"yes\"" .($user["ambassador"] == "yes" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_yes']." <input type=\"radio\" name=\"ambassador\" value=\"no\"" .($user["ambassador"] == "no" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_no'], 1);
    tr($lang_userdetails['row_ambassadorfor'], "<textarea cols=\"60\" rows=\"6\" name=\"ambassadorfor\">".$ambassadorfor."</textarea>", 1);

tr($lang_userdetails['row_artmember'], "<input type=\"radio\" name=\"artmember\" value=\"yes\"" .($user["artmember"] == "yes" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_yes']." <input type=\"radio\" name=\"artmember\" value=\"no\"" .($user["artmember"] == "no" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_no'], 1);
	tr($lang_userdetails['row_artmemberfor'], "<textarea cols=\"60\" rows=\"6\" name=\"artmemberfor\">".$artmemberfor."</textarea>", 1);
//服务组标签 end
	...
	//if (get_user_class() >= $cruprfmanage_class || $user["id"] == $CURUSER["id"])
	if (get_user_class() >= $cruprfmanage_class && $user["class"] < get_user_class() || $user["id"] == $CURUSER["id"])
	{
		begin_frame($lang_userdetails['text_delete_user'], true);
		print("<form method=\"post\" action=\"delacctadmin.php\" name=\"deluser\">");
		if(get_user_class() < $cruprfmanage_class)
		    print("请输入密码以确认删除：<input type=\"password\" name=\"password\" style=\"width: 180px; border: 1px solid gray\"/>");
		print("<input name=\"userid\" size=\"10\" type=\"hidden\" value=\"". $user["id"] ."\" />
		<input name=\"delenable\" type=\"checkbox\" onclick=\"if (this.checked) {enabledel('".$lang_userdetails['js_delete_user_note']."');}else{disabledel();}\" /><input name=\"submit\" type=\"submit\" value=\"".$lang_userdetails['submit_delete']."\" disabled=\"disabled\" /></form>");
		end_frame();
	}
end_main_frame();
